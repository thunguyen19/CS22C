...
private:
	...
	void inOrderPrintHelper(NodePtr root, ofstream &outfile);
	//private helper function for inOrderPrint
	//recursively prints tree values in order from smallest to largest

	void preOrderPrintHelper(NodePtr root, ofstream &outfile);
	//private helper function for preOrderPrint
	//recursively prints tree values in preorder

	void postOrderPrintHelper(NodePtr root, ofstream &outfile);
...
template<class bstitem>
void BinarySearchTree<bstitem>::inOrderPrint(ofstream &outfile) {
	if (root == NULL) {
		outfile << "Empty tree.\n";
	} else
		inOrderPrintHelper(root, outfile);
	outfile << "\n";
}

template<class bstitem>
void BinarySearchTree<bstitem>::inOrderPrintHelper(NodePtr root, ofstream &outfile) {
	if (root != NULL) {
		inOrderPrintHelper(root->left, outfile);
		outfile << root->data << " ";
		inOrderPrintHelper(root->right, outfile);
	}
}

template<class bstitem>
void BinarySearchTree<bstitem>::preOrderPrint(ofstream &outfile) {
	if (root == NULL) {
		outfile << "Empty tree.\n";
	} else
		preOrderPrintHelper(root, outfile);
	outfile << "\n";
}

template<class bstitem>
void BinarySearchTree<bstitem>::preOrderPrintHelper(NodePtr root, ofstream &outfile) {
	if (root != NULL) {
		outfile << root->data << " ";
		preOrderPrintHelper(root->left, outfile);
		preOrderPrintHelper(root->right, outfile);
	}
}

template<class bstitem>
void BinarySearchTree<bstitem>::postOrderPrint(ofstream &outfile) {
	if (root == NULL) {
		outfile << "Empty tree.\n";
	} else {
		postOrderPrintHelper(root, outfile);
		outfile << "\n";
	}
}

template<class bstitem>
void BinarySearchTree<bstitem>::postOrderPrintHelper(NodePtr root, ofstream &outfile) {
	if (root != NULL) {
		postOrderPrintHelper(root->left, outfile);
		postOrderPrintHelper(root->right, outfile);
		outfile << root->data << " ";
	}
}
